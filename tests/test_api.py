...
def test_status_returns_404_for_missing_task(client):
    response = client.get("/api/status/missing-task")
...
def test_status_includes_failure_error(client, app):
    with app.app_context():
        task = TaskSession(id="task-1", video_path="video.mp4", status="FAILED", progress=20, error_message="bad video")
        db.session.add(task)
        db.session.commit()
...
def test_result_returns_success_payload(client, app):
    ...
    response = client.get("/api/result/task-2")
...
def test_result_includes_audit_data_from_csv(client, app):
    ...
    response = client.get("/api/result/task-3")
...
def test_result_tolerates_missing_csv(client, app):
    ...
    response = client.get("/api/result/task-4")
...
def test_result_keeps_core_payload_compatible(client, app):
    ...
    response = client.get("/api/result/task-5")
